 ...
 * @author Yongshun Ye
 */
public abstract class PostLightweightService extends LightweightService {
    @Override
    public FullResponse onServeFullRequest(FullRequest fullRequest) throws Exception {
        if (!fullRequest.method().equals(Method.POST))
            return new DefaultOutboundFullResponse(HttpResponseStatus.METHOD_NOT_ALLOWED);

        return onServePost(fullRequest);
    }

    /**
     * serves a full request with a "POST" request method.
     *
     * @param fullRequest the full request with a "POST" request method.
     * @return the full response to send back
     * @throws Exception when an exception occurred
     */
    public abstract FullResponse onServePost(FullRequest fullRequest) throws Exception;
}
